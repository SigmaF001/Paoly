    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(strings.scanError)));
      }
    } finally {
              ...
              TextButton(
                onPressed: () {
                  ...
                  if (remaining.isNotEmpty) {
                    setState(() => _selectedCategoryId = remaining.first.id);
                  }
                  Navigator.of(ctx).pop();
                },
                child: Text(
                  strings.deleteCategory,
                  style: GoogleFonts.notoSansThai(color: AppColors.expense),
                ),
              ),
            TextButton(
              onPressed: () => Navigator.of(ctx).pop(),
